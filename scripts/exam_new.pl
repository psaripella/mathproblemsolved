...
print "Usage: perl exam.pl <search_text_file> <product> <start_probem> <end_problem> \n";
...
if (-e $base_path) {
  ...
  $problem_string = q?<p>Solve the following: </p>?.q?<p><img src="?.$question_directory.q?/new_question?.${i}.q?.jpg" alt="" /></p>,?;
}
else
{
...
$random0thru3 = int(rand(4));

#print " random number is $random0thru3 \n";
switch($random0thru3){
   case 0            { $correct_string = 'A' ; $incorrect_string = 'B | C | D'; $first = 1; $second = 2; $third = 3; $fourth = 4; }
   case 1            { $correct_string = 'B' ; $incorrect_string = 'A | C | D'; $first = 2; $second = 1; $third = 3; $fourth = 4; }
   case 2            { $correct_string = 'C' ; $incorrect_string = 'A | B | D'; $first = 3; $second = 2; $third = 1; $fourth = 4; }
   case 3            { $correct_string = 'D' ; $incorrect_string = 'A | B | C'; $first = 4; $second = 1; $third = 2; $fourth = 1; }
   else              { print "Bad random number which is not 0 or 1 or 2 or 3" }
} ;

#  $first = $random0thru3 + 1 ;
#  $second = ($first % 4) + 1;
#  $third  = ($second % 4) + 1;
#  $fourth = ($third % 4) + 1 ;

   $result_text =  q?<p><img src="?.$options_directory.q?/option?.${i}.q?_1.jpg" alt="" /></p>?;
  $option1_string =  q?<p>A:<img src="?.$options_directory.q?/option?.${i}.q?_?.$first.q?.jpg" alt="" /></p>?;
  $option2_string =  q?<p>B:<img src="?.$options_directory.q?/option?.${i}.q?_?.$second.q?.jpg" alt="" /></p>?;
  $option3_string =  q?<p>C:<img src="?.$options_directory.q?/option?.${i}.q?_?.$third.q?.jpg" alt="" /></p>?;
  $option4_string =  q?<p>D:<img src="?.$options_directory.q?/option?.${i}.q?_?.$fourth.q?.jpg" alt="" /></p>?;
  $option234_string =  $option2_string.$option3_string.$option4_string;
...
#  $problem_string = q?(?.$sql_id.q?,?.$asset_id.q?, '?. $search_txt.q?', '?.$latex_txt.q?', '<p>&nbsp;?.$search_txt.q?</p>\r\n', '\r\n<p>&nbsp;</p>?;
  ...
  print "$problem_string"."$option1_string"."$option234_string," ;
