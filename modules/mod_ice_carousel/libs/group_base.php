...
jimport('joomla.filesystem.folder');
  class IceGroupBase{
     ...
     * @return boolean.
     */
     function renderThumb( $path, $width=100, $height=100, $title='', $isThumb=true, $image_quanlity = 100, $returnPath = false, $resizeUp = false, $alwayRender = false ){
      ...
      if( $isThumb ){
        ...
        if( file_exists($imagSource)  ) {
		  ...
		  if( file_exists($thumbPath) && $alwayRender){
			@JFile::delete($thumbPath);
		  }
          if( !file_exists($thumbPath) ) {
            $thumb = PhpThumbFactory::create( $imagSource  );
			$thumb->setOptions( array('jpegQuality'=> $image_quanlity,'resizeUp'=>$resizeUp) );
