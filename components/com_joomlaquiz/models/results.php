...
{	
	public function getResults(){
		...
		$database->setQuery("SELECT `c_id` FROM `#__quiz_t_quiz` WHERE `c_id` <> 0");
		$quiz_ids = $database->loadColumn();

		$quiz_access_ids = array();
		$quiz_access_cert = array();
		if(count($quiz_ids)){
			foreach ($quiz_ids as $quiz_id) {
				$resultAccessGranted = $my->authorise('core.result', 'com_joomlaquiz.quiz.'.$quiz_id);
				$certAccessGranted = $my->authorise('core.certificate', 'com_joomlaquiz.quiz.'.$quiz_id);
				if($resultAccessGranted){
					$quiz_access_ids[] = $quiz_id;
					$quiz_access_cert[$quiz_id] = $certAccessGranted;
				}
			}

			if(empty($quiz_access_ids)){
				return 'no_access';
			} else {
				$quiz_and = " AND q.`c_id` IN (".implode(",", $quiz_access_ids).")";
			}
		...
		$query = "SELECT sq.unique_id, sq.c_id as id, SUM(squ.c_score) as user_score, sq.c_passed, sq.c_total_score, sq.c_total_time, sq.c_date_time, sq.c_passing_score AS sq_c_passing_score, sq.c_quiz_id, "
					. "\n q.c_id, q.c_title, q.c_author, q.c_passing_score, sq.c_student_id, u.username, u.name, u.email, q.c_full_score, q.c_pool, ch.q_chain, q.c_grading, q.c_certificate "
					...
					. "\n LEFT JOIN #__quiz_t_quiz as q ON sq.c_quiz_id = q.c_id ".$quiz_and." WHERE c_student_id = '{$my->id}' "
		...
		for($i=0, $n=count($tmp_rows); $i<$n; $i++) {	
			if ($tmp_rows[$i]->c_grading && !in_array($tmp_rows[$i]->c_quiz_id, $gquizzes)) {
				if ($tmp_rows[$i]->c_grading == 1) {//First attempt
					$query = "SELECT sq.unique_id, sq.c_id  as id, sq.c_passed, sq.c_total_score, sq.c_total_time, sq.c_date_time, sq.c_passed, sq.c_passing_score AS sq_c_passing_score, sq.c_quiz_id, "
								. "\n q.c_id, q.c_title, q.c_author, q.c_passing_score, sq.c_student_id, u.username, u.name, u.email, q.c_full_score, q.c_pool, ch.q_chain, q.c_grading, q.c_certificate"
								. "\n FROM #__quiz_r_student_quiz as sq"
								. "\n LEFT JOIN #__users as u ON sq.c_student_id = u.id"
								. "\n LEFT JOIN `#__quiz_q_chain` AS ch ON ch.s_unique_id = sq.unique_id "
					...
					$query = "SELECT sq.unique_id, sq.c_id  as id, sq.c_passed, sq.c_total_score, sq.c_total_time, sq.c_date_time, sq.c_passing_score AS sq_c_passing_score, sq.c_quiz_id, "
								. "\n q.c_id, q.c_title, q.c_author, q.c_passing_score, sq.c_student_id, u.username, u.name, u.email, q.c_full_score, q.c_pool, ch.q_chain, q.c_grading, q.c_certificate "
								. "\n FROM #__quiz_r_student_quiz as sq"
								. "\n LEFT JOIN #__users as u ON sq.c_student_id = u.id"
								. "\n LEFT JOIN `#__quiz_q_chain` AS ch ON ch.s_unique_id = sq.unique_id"
					...
					$query = "SELECT sq.unique_id, MAX(sq.c_id) AS c_id, sq.c_passed, AVG(sq.c_total_score) AS c_total_score, sq.c_total_time, sq.c_date_time, sq.c_passed, sq.c_passing_score AS sq_c_passing_score, sq.c_quiz_id, "
								. "\n q.c_id, q.c_title, q.c_author, q.c_passing_score, sq.c_student_id, u.username, u.name, u.email, q.c_full_score, q.c_pool, ch.q_chain, q.c_grading, q.c_certificate "
								. "\n FROM #__quiz_r_student_quiz as sq"
								. "\n LEFT JOIN #__users as u ON sq.c_student_id = u.id"
								. "\n LEFT JOIN `#__quiz_q_chain` AS ch ON ch.s_unique_id = sq.unique_id"
	...
	public function getQuizParams(){
		...
		if(!$is_share){
			...
			$query = "SELECT * FROM #__quiz_r_student_quiz WHERE c_id = '{$stu_id}'";
			$database->SetQuery($query);
			$result_data = $database->LoadObjectList();
		}
		...
		if ($doing_quiz == 1) {
			...
			$quiz_params[0]->share_id = ($share_id != '') ? $share_id : 0;
			$_SESSION['share_id'] = $quiz_params[0]->share_id;
