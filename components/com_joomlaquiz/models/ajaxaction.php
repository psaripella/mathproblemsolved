...
{	
			...
			if ($stu_quiz_id) {
				...
				if ($cur_tmpl) {
					...
					if (!$c_manual) {	
						...
						$params['started'] = date('Y-m-d H:i:s', strtotime($quiz_time2));
	...
	public function JQ_QuestPreview() {
		...
		if (($quiz_id) && ($quiz_id == $q_quiz) && ($preview_id == $preview_code) && ($quest_id)) {
			$query = "SELECT * FROM #__quiz_t_question WHERE c_quiz_id = '".$quiz_id."' AND c_id = '".$quest_id."' AND published = 1";
			$database->SetQuery($query);
			$q_data = $database->LoadObjectList();
	...
	function JQ_NextPreview() {
		...
		if (isset($quest_ids[0])) {
			$query = "SELECT c_quiz_id FROM #__quiz_t_question WHERE c_id = '".$quest_ids[0]."' AND published = 1";
			$database->SetQuery( $query );
			$q_quiz = $database->LoadResult();
		}
		...
		if ($cur_tmpl && ($quiz_id) && ($quiz_id == $q_quiz) && ($preview_id == $preview_code) && count($quest_ids)) {
	...
	public function JQ_GetQuestData($q_data, $i_quiz_id, $stu_quiz_id = 0) {
		...
		$query = "SELECT `c_random`, `c_pool`, `c_pagination`, `c_enable_skip` FROM `#__quiz_t_quiz` WHERE `c_id` = '$i_quiz_id'";
		$database->SetQuery($query);
	...
	public function feedback($database, $blank_fbd, $blank_fbd_count, $answer, $quest_id, $c_quest_cur_attempt, $c_all_attempts, $quiz, $question, $qtype, $is_no_attempts, $is_correct, $jq_language, $got_one_correct, $questtype1_answer_incorrect, $is_avail){
		
        $ret_str = "\t" . '<feedback>' . "\n";
		
		$database->SetQuery("SELECT a.template_name FROM #__quiz_templates as a, #__quiz_t_quiz as b WHERE b.c_id = '".$quiz->c_id."' and b.c_skin = a.id");
		$cur_template = $database->LoadResult();
