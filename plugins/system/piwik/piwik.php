...
class plgSystemPiwik extends JPlugin
{
    public function onAfterRender()
    {
        ...
        // Other methods to skip AJAX-calls
        if (JRequest::getCmd('format') == 'raw') return false;
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') return false;

        // Load parameters
        $siteId = $this->getParams()->get('site_id', 1);
        $piwikUrl = $this->getParams()->get('piwik_url');
        $tokenAuth = $this->getParams()->get('token_auth');
        $skipRobots = $this->getParams()->get('skip_robots', 0);
        ...
        if(empty($piwikUrl) || empty($tokenAuth)) return false;
        ...
        if(!empty($_SERVER['HTTP_CLIENT_IP'])) {
            ...
            $piwikTracker->setIp($_SERVER['HTTP_CF_CONNECTING_IP']);

        } elseif(!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
            $iplist = explode(',', $_SERVER['HTTP_X_FORWARDED_FOR']);
            $piwikTracker->setIp(array_shift($iplist));
        }
